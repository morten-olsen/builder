  ...
  >
    ...
    <BaseSelect.Portal>
      <BaseSelect.Positioner className="z-50" sideOffset={4}>
        <BaseSelect.Popup className="max-h-64 overflow-y-auto rounded border border-border-bright bg-surface-2 py-1 shadow-lg shadow-black/40">
          {options.map((opt) => (
            <BaseSelect.Item
              key={opt.value}
              value={opt.value}
              disabled={opt.disabled}
              className="flex cursor-pointer items-center gap-2 px-3 py-1.5 font-mono text-sm text-text-base outline-none data-[highlighted]:bg-surface-3 data-[highlighted]:text-text-bright data-[selected]:text-accent-bright data-[disabled]:opacity-40"
            >
