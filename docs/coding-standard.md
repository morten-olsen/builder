...
// ❌ Bad - .ts extension when file is .ts
import { createUser } from './user-service.ts';
```

**Rationale**:

- Modern ES modules require explicit extensions for proper resolution
...
Use Node.js `node:fs/promises` for file operations:
...
Use Node.js `node:crypto` for hashing:
...
// Password hashing (use scrypt for passwords)
...
- [ ] Including `.ts` extensions in all imports
...
- [ ] Tests use `node:test` imports with `assert`
- [ ] Using `#` for private class fields
